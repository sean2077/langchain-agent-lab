...
- 模型生成且会被 GFM 激活的链接目标不会进入正文：内联/引用链接只保留 label，autolink 与
  ...
  首轮提示直接对齐逐 block 引用契约，必要时最多做一次无工具修订。修订后满足契约属于已
  恢复的内部步骤；成功的链接清洗和 citation 规范化也不进入面向用户的 warnings。最终仍不
  满足引用契约时由报告校验 fail closed，语言修订后仍不匹配时才保留明确 warning。
- Clash/Mihomo Fake-IP 场景使用不继承 proxy 环境的公共 DNS 再验证并固定到公网 IP，不直接放行
  `198.18.0.0/15`；这与后续直连并固定公网 IP 的 reader 使用同一网络边界。A/AAAA 查询各允许
  一次有界重试，任一地址族成功即可继续；两族都失败时仍 fail closed，且不缓存 DNS 结果。
