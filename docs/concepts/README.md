...
| 问题 | 一句话判断 | 详细答案 |
| --- | --- | --- |
| 搜索候选与已读证据有什么区别？为什么要重新验证重定向后的最终 URL？ | 候选只是待核验线索；只有安全读取成功后的最终页面才是可引用证据。 | [来源候选、已读证据与最终 URL](source-provenance.md) |
...
| HTTPX transport timeout 与 Agent wall-clock deadline 有什么区别？ | transport timeout 限制单次 I/O 阶段的等待；deadline 限制整次研究从开始到结束的总时长。 | [传输超时与总时限](transport-timeouts.md) |
